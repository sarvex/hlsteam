...
HL_PRIM bool HL_NAME(send_p2p_packet)( vuid uid, vbyte *data, int length, int type, int channel ) {
	return SteamNetworking()->SendP2PPacket(hl_to_uid(uid),data,length,(EP2PSend)type,channel);
}

HL_PRIM bool HL_NAME(accept_p2p_session)( vuid uid ) {
	return SteamNetworking()->AcceptP2PSessionWithUser(hl_to_uid(uid));
}

HL_PRIM bool HL_NAME(is_p2p_packet_available)( uint32 *msgSize, int channel ) {
	return SteamNetworking()->IsP2PPacketAvailable(msgSize,channel);
}

HL_PRIM vuid HL_NAME(read_p2p_packet)( vbyte *data, int maxLength, uint32 *length, int channel ) {
	CSteamID uid;
	if( !SteamNetworking()->ReadP2PPacket(data, maxLength, length, &uid, channel) )
		return NULL;
...
HL_PRIM vdynamic *HL_NAME(get_p2p_session_data)( vuid uid ) {
	...
	if( !SteamNetworking()->GetP2PSessionState(hl_to_uid(uid),&state) )
		return NULL;
	HLValue v;
...
HL_PRIM bool HL_NAME(close_p2p_session)( vuid uid ) {
	return SteamNetworking()->CloseP2PSessionWithUser(hl_to_uid(uid));
}
